...
from wtforms import StringField, IntegerField, SelectField, TextAreaField, DateField, SubmitField, BooleanField, PasswordField, ValidationError
...
class User(db.Model):
    ...
    # Made nullable=True to support existing users created before this column existed
...
class Venue(db.Model):
    ...
    address_link = db.Column(db.String(500))  # 2GIS link for directions
...
class BookingForm(FlaskForm):
    ...
    client_password = PasswordField('Create Password', 
                                   validators=[DataRequired(), validate_password_strength],
                                   render_kw={'placeholder': 'Min 8 chars, 1 uppercase, 1 lowercase, 1 digit, 1 special char'})
...
class AddOnForm(FlaskForm):
    ...
    client_phone = StringField('Phone Number', validators=[DataRequired()],
                               render_kw={'placeholder': '+7 700 123 4567'})
    client_password = PasswordField('Account Password', validators=[DataRequired()], 
                                   render_kw={'placeholder': 'Enter your account password'})
    booking_id = SelectField('Select Booking', coerce=int)
    selected_date = StringField('Selected Date')
...
class PaymentForm(FlaskForm):
    ...
    # Generate year choices dynamically starting from the current year
...
class GuestRSVPForm(FlaskForm):
    ...
    # ========== SHOW PROGRAMS ROUTES ==========

# Add this form class after MusicianFilterForm
class ShowProgramFilterForm(FlaskForm):
    program_type = StringField('Program Type')
    city = StringField('City')
    max_price = IntegerField('Max Price (KZT)', validators=[NumberRange(min=0, max=10000000)])
    submit = SubmitField('Search Programs')

# Add these routes after the musician routes

@app.route('/show-programs')
def show_programs():
    """Display list of show programs from CSV with filters."""
    form = ShowProgramFilterForm()
    program_records = load_csv_records('show_programs.csv')
    ...
    for r in program_records:
        if program_type and program_type.lower() not in (r.get('program_type', '') or '').lower():
            continue
    ...
    return render_template('show_programs.html', programs=filtered, form=form)


@app.route('/show-program/<id>')
def show_program_detail(id):
    programs = load_csv_records('show_programs.csv')
    program = next((p for p in programs if str(p.get('id')) == str(id)), None)
    if not program:
        return redirect(url_for('show_programs'))
    return render_template('show_program_detail.html', program=program)


@app.route('/book_show_program/<id>', methods=['GET', 'POST'])
def book_show_program(id):
    from flask import session

    programs = load_csv_records('show_programs.csv')
    program = next((p for p in programs if str(p.get('id')) == str(id)), None)
    if not program:
        flash('Selected show program not found.', 'error')
        return redirect(url_for('show_programs'))

    form = AddOnForm()

    # Build booking choices for this user (if any)
    booking_choices = []
    user = None

    if request.method == 'POST':
        submitted_email = request.form.get('client_email', '').strip()
        submitted_phone = request.form.get('client_phone', '').strip()
        normalized_phone = normalize_phone_number(submitted_phone) if submitted_phone else None
        ...
        if user:
            ...
            if submitted_email:
                query = query.filter_by(client_email=submitted_email)
            user_bookings = query.order_by(Booking.event_date.desc(), Booking.created_at.desc()).all()
            for b in user_bookings:
                label = f"{b.venue.name} — {b.event_date.strftime('%Y-%m-%d')} ({b.event_type})"
                booking_choices.append((b.id, label))

    elif request.method == 'GET' and 'user_id' in session:
        user = User.query.get(session['user_id'])
        if user:
            user_bookings = Booking.query.filter_by(user_id=user.id).order_by(
                Booking.event_date.desc(), Booking.created_at.desc()
            ).all()
            for b in user_bookings:
                label = f"{b.venue.name} — {b.event_date.strftime('%Y-%m-%d')} ({b.event_type})"
                booking_choices.append((b.id, label))
            form.client_email.data = user.email
            form.client_phone.data = user.phone

    form.booking_id.choices = booking_choices

    if form.validate_on_submit():
        # Normalize phone number and verify user exists and password is correct
        normalized_phone = normalize_phone_number(form.client_phone.data)
        user = User.query.filter_by(phone=normalized_phone).first()
        if not user or not check_password_hash(user.password, form.client_password.data):
            flash('Invalid phone number or password. Please use your booking credentials.', 'error')
            return render_template('book_show_program.html', program=program, form=form)

        if not form.booking_id.data:
            flash('Please select which venue booking to attach this show program to.', 'warning')
            return render_template('book_show_program.html', program=program, form=form)

        booking = Booking.query.filter_by(id=form.booking_id.data, user_id=user.id).first()
        if not booking:
            flash('No matching venue booking found. Please check your details or book a venue first.', 'info')
        ...
        note_parts.append(f"Added Show Program: {program.get('name', 'Program')} (+{add_price} KZT)")
        booking.special_requests = '\n'.join([p for p in note_parts if p])
        db.session.commit()

        flash('Show program has been added to your venue booking and total updated.', 'success')
        return redirect(url_for('user_profile', user_id=user.id))

    return render_template('book_show_program.html', program=program, form=form)

# Helper functions
def find_or_create_user(name, email, phone, password=None):
    """Find existing user by email or phone, or create new one with password"""
    # Normalize phone number before search/save
    normalized_phone = normalize_phone_number(phone)
    
    # Try to find user by email or normalized phone
...
def load_csv_records(csv_filename):
    ...
    records = []
    try:
        # Try multiple possible paths so the app works no matter
        # which directory you run it from.
        #
        # Priority:
        # 1) ./instance/<file> relative to current working directory
        # 2) instance folder next to this app.py
        # 3) instance folder one level above this app.py (project root)
        # 4) CSV in current working directory
        possible_paths = []

        # 1) CWD/instance/<file>
        possible_paths.append(os.path.join('instance', csv_filename))

        # 2) instance next to this file
        base_dir = os.path.abspath(os.path.dirname(__file__))
        possible_paths.append(os.path.join(base_dir, 'instance', csv_filename))

        # 3) instance one level above this file (e.g. project root)
        parent_dir = os.path.abspath(os.path.join(base_dir, os.pardir))
        possible_paths.append(os.path.join(parent_dir, 'instance', csv_filename))

        # 4) raw filename in CWD (fallback)
        possible_paths.append(csv_filename)

        instance_csv_path = next((p for p in possible_paths if os.path.exists(p)), None)
...
def host_detail(id):
    ...
    # Build dynamic 60-day availability window starting from today
    today = date.today()
    horizon = today + timedelta(days=60)
    all_days = [today + timedelta(days=i) for i in range((horizon - today).days + 1)]

    # Mark a date as booked if there is a booking on that date whose
    # special_requests mention this host's name.
    booked_dates = set()
    bookings = (
        Booking.query
        .filter(Booking.event_date >= today, Booking.event_date <= horizon)
        .filter(Booking.special_requests.contains(host.get('name', '')))
        .all()
    )
    for b in bookings:
        booked_dates.add(b.event_date)

    # If there are no real bookings yet, randomly mark a few days as "already booked"
    if not booked_dates and all_days:
        # Pick up to 3 random dates from the window
        random_count = min(3, len(all_days))
        for d in random.sample(all_days, random_count):
            booked_dates.add(d)

    available_dates = [d for d in all_days if d not in booked_dates]

    # Convert to ISO strings for templates/JS
    available_iso = [d.isoformat() for d in available_dates]
    booked_iso = [d.isoformat() for d in sorted(booked_dates)]

    return render_template('host_detail.html',
                           host=host,
                           available_dates=available_iso,
                           booked_dates=booked_iso)
...
def musician_detail(id):
    ...
    bookings = (
        Booking.query
        .filter(Booking.event_date >= today, Booking.event_date <= horizon)
        .filter(Booking.special_requests.contains(artist.get('name', '')))
        .all()
    )
    for b in bookings:
        booked_dates.add(b.event_date)
    ...
    return render_template('musician_detail.html',
                           musician=artist,
                           available_dates=available_iso,
                           booked_dates=booked_iso)
...
def book_venue(venue_id):
    ...
    # If event_type comes from filters in the URL, use it ONLY to pre-select on initial GET.
    # On POST we let WTForms keep whatever the user selected in the form.
    if request.method == 'GET':
        selected_event_type = request.args.get('event_type')
        if selected_event_type and any(value == selected_event_type for value, _ in form.event_type.choices):
            form.event_type.data = selected_event_type
    
    if form.validate_on_submit():
        # Validate guest count against selected hall and venue capacity
        selected_hall_obj = None
        if form.selected_hall_id.data and form.selected_hall_id.data != 0:
            selected_hall_obj = Hall.query.get(form.selected_hall_id.data)
        hall_capacity = selected_hall_obj.capacity if selected_hall_obj else venue.capacity_max
        max_hall_capacity = max((hall.capacity for hall in venue.halls), default=venue.capacity_max)
        allowed_max_display = hall_capacity if selected_hall_obj else max_hall_capacity

        if form.guest_count.data < venue.capacity_min or form.guest_count.data > hall_capacity or form.guest_count.data > venue.capacity_max:
            flash(f"Guest count exceeds capacity. Allowed: {venue.capacity_min}-{allowed_max_display} guests.", 'error')
            # Ensure the template has correct max capacity on re-render
            return render_template('book_venue.html', venue=venue, form=form, max_hall_capacity=max_hall_capacity)
        ...
        session['booking_data'] = {
            ...
            'client_password': form.client_password.data,  # Store password for user creation
    ...
    # Compute the true maximum capacity across halls (fallback to venue.capacity_max)
    max_hall_capacity = max((hall.capacity for hall in venue.halls), default=venue.capacity_max)
    return render_template('book_venue.html', venue=venue, form=form, max_hall_capacity=max_hall_capacity)
...
def payment_confirmation():
    ...
    if form.validate_on_submit():
        # Additional server-side payment validation
        # 1) Cardholder name should contain real letters, not just digits or symbols
        holder_name = (form.card_holder.data or '').strip()
        if not holder_name or not re.search(r'[A-Za-z]', holder_name) or re.fullmatch(r'\d+', holder_name):
            flash('Please enter the real cardholder name using letters (as shown on the card).', 'error')
        ...
        # 2) Expiry month/year must not be in the past
        try:
            exp_month = int(form.expiry_month.data)
            exp_year = int(form.expiry_year.data)
            now = dt.now()
            if exp_year < now.year or (exp_year == now.year and exp_month < now.month):
                flash('The card expiry date is in the past. Please use a valid card.', 'error')
                return render_template('payment_confirmation.html', 
                                     booking_data=booking_data, 
                                     venue=venue, 
                                     selected_hall=selected_hall,
            ...
            flash('Please select a valid expiry month and year.', 'error')
            return render_template('payment_confirmation.html', 
                                 booking_data=booking_data, 
                                 venue=venue, 
                                 selected_hall=selected_hall,
        ...
        user = find_or_create_user(
            ...
            password=booking_data['client_password']
        )
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, password):
            ...
            flash('Invalid phone number or password. Please check your credentials or make a booking first.', 'warning')
...
def forgot_password():
    """Forgot password page - verify user identity"""
    from flask import session
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(phone=normalized_phone, email=email).first()
        ...
        if user:
            ...
            flash('No account found with this phone number and email combination. Please check your details.', 'error')
...
def user_profile(user_id):
    ...
    # Check if user is logged in via session and matches the requested user_id
    if 'user_id' in session and session['user_id'] == user_id:
        user = User.query.get_or_404(user_id)
    else:
        # If not logged in or user_id doesn't match, redirect to profile login
    ...
    # Separate bookings by status - exclude cancelled bookings from both lists
    upcoming_bookings = [b for b in bookings if b.event_date >= date.today() and b.status in ['pending', 'confirmed']]
    past_bookings = [b for b in bookings if b.event_date < date.today() and b.status != 'cancelled']
    ...
    # Calculate invitation statistics and guest lists for each booking
    booking_stats = {}
    booking_guest_lists = {}
    for booking in bookings:
        if booking.invitations:
            invitation = booking.invitations[0]  # Get the first invitation
            guests_q = InvitedGuest.query.filter_by(invitation_id=invitation.id)
            
            total_invited = guests_q.count()
            attending = guests_q.filter_by(rsvp_status='attending').count()
            not_attending = guests_q.filter_by(rsvp_status='not_attending').count()
            ...
            # Build simple name lists per status for display in profile
            guests = guests_q.order_by(InvitedGuest.created_at.desc()).all()
            booking_guest_lists[booking.id] = {
                'attending': [g.name for g in guests if g.rsvp_status == 'attending'],
                'not_attending': [g.name for g in guests if g.rsvp_status == 'not_attending'],
                'pending': [g.name for g in guests if g.rsvp_status not in ['attending', 'not_attending']]
            }
    ...
    return render_template(
        ...
        booking_guest_lists=booking_guest_lists
    )

@app.route('/booking/<int:booking_id>/cancel', methods=['POST'])
def cancel_booking(booking_id):
    """Allow a logged-in user to cancel their own upcoming booking."""
    from flask import session
    ...
    # Only allow cancellation for upcoming, non-cancelled events
    if booking.status == 'cancelled':
        flash('This booking is already cancelled.', 'info')
        return redirect(url_for('user_profile', user_id=user_id))

    if booking.event_date < date.today():
        flash('You cannot cancel a past event.', 'warning')
        return redirect(url_for('user_profile', user_id=user_id))
    ...
    flash('Your booking has been cancelled. Your deposit will be returned as soon as possible.', 'success')
    return redirect(url_for('user_profile', user_id=user_id))


@app.route('/booking/<int:booking_id>/remove_addon', methods=['POST'])
def remove_booking_addon(booking_id):
    """
    Remove a single add-on (host, musician or show program) from a user's booking.
    The add-ons are stored as individual text lines in booking.special_requests.
    """
    from flask import session

    user_id = session.get('user_id')
    if not user_id:
        flash('Please log in to manage your booking.', 'warning')
        return redirect(url_for('profile'))

    booking = Booking.query.get_or_404(booking_id)

    # Security: booking must belong to the current user
    if booking.user_id != user_id:
        flash('You can only modify your own bookings.', 'error')
        return redirect(url_for('user_profile', user_id=user_id))

    line_text = (request.form.get('addon_line') or '').strip()
    if not line_text or not booking.special_requests:
        flash('Nothing to remove from this booking.', 'info')
        return redirect(url_for('user_profile', user_id=user_id))

    # Split stored notes into individual non-empty lines
    original_lines = [l for l in booking.special_requests.split('\n') if l.strip()]
    updated_lines = []
    removed = False
    price_delta = 0

    for l in original_lines:
        if not removed and l.strip() == line_text:
            # Try to extract price from pattern "(+12345 KZT)"
            match = re.search(r'\+(\d+)\s*KZT', l)
            if match:
                try:
                    price_delta = int(match.group(1))
                except Exception:
                    price_delta = 0
            removed = True
            continue
    ...
    if not removed:
        flash('Could not find this add-on in your booking.', 'error')
        return redirect(url_for('user_profile', user_id=user_id))

    booking.special_requests = '\n'.join(updated_lines)

    # Adjust total amount if we successfully extracted a price
    if price_delta and booking.total_amount:
        booking.total_amount = max(0, booking.total_amount - price_delta)

    db.session.commit()

    flash('The selected add-on has been removed from your booking and total updated.', 'success')
    return redirect(url_for('user_profile', user_id=user_id))
...
def cleanup_old_bookings():
    """
    Delete bookings whose event_date is more than 3 days in the past
    together with their dependent records (guests and invitations).
    """
    try:
        cutoff = date.today() - timedelta(days=3)
        old_bookings = Booking.query.filter(Booking.event_date < cutoff).all()

        if not old_bookings:
            return

        for booking in old_bookings:
            # Delete invitation guests then invitations
            if booking.invitations:
                for invitation in list(booking.invitations):
                    InvitedGuest.query.filter_by(invitation_id=invitation.id).delete()
                    db.session.delete(invitation)

            # Delete direct guests
            Guest.query.filter_by(booking_id=booking.id).delete()
        ...
        # Log and continue – we don't want cleanup failure to break the app
        print(f"Error during old bookings cleanup: {e}")


@app.before_request
def _run_scheduled_cleanup():
    """
    Lightweight hook to ensure outdated bookings are periodically cleaned up.
    Runs on every request but usually exits quickly when there is nothing to delete.
    """
...
def book_host(id):
    ...
    # Build a 60-day availability window for calendar selection
    today = date.today()
    horizon = today + timedelta(days=60)
    all_days = [today + timedelta(days=i) for i in range((horizon - today).days + 1)]
    ...
    # On POST we use submitted phone/email to look up potential user & bookings
    if request.method == 'POST':
        submitted_email = request.form.get('client_email', '').strip()
        submitted_phone = request.form.get('client_phone', '').strip()
        normalized_phone = normalize_phone_number(submitted_phone) if submitted_phone else None
        ...
        if normalized_phone:
            user = User.query.filter_by(phone=normalized_phone).first()
        # If user is already in session, prefer that
        if not user and 'user_id' in session:
            user = User.query.get(session['user_id'])

        if user:
            # All bookings for this user (optionally filtered by email)
            query = Booking.query.filter_by(user_id=user.id)
            if submitted_email:
                query = query.filter_by(client_email=submitted_email)
    ...
    # On GET we can pre-fill choices based on logged in user, if any
    elif request.method == 'GET' and 'user_id' in session:
        user = User.query.get(session['user_id'])
        if user:
            user_bookings = Booking.query.filter_by(user_id=user.id).order_by(
            ...
            for b in user_bookings:
                label = f"{b.venue.name} — {b.event_date.strftime('%Y-%m-%d')} ({b.event_type})"
                booking_choices.append((b.id, label))
            # Helper: prefill contact fields from profile
            form.client_email.data = user.email
            form.client_phone.data = user.phone
    ...
    if form.validate_on_submit():
        ...
        if not user or not check_password_hash(user.password, form.client_password.data):
            ...
            return render_template('book_host.html', host=host, form=form,
                                   available_dates=available_iso, booked_dates=booked_iso)

        # If user has multiple bookings, require explicit selection
        if not form.booking_id.data:
            flash('Please select which venue booking to attach this host to.', 'warning')
            return render_template('book_host.html', host=host, form=form,
                                   available_dates=available_iso, booked_dates=booked_iso)
        ...
        if booking.special_requests:
            note_parts.append(booking.special_requests)
        sel_date = (form.selected_date.data or '').strip()
        note_parts.append(f"Added Host: {host.get('name', 'Host')} (+{add_price} KZT){' on ' + sel_date if sel_date else ''}")
        booking.special_requests = '\n'.join([p for p in note_parts if p])
        db.session.commit()
...
def book_musician(id):
    ...
    if form.validate_on_submit():
        ...
        if not form.booking_id.data:
            flash('Please select which venue booking to attach this musician to.', 'warning')
            return render_template('book_musician.html', musician=artist, form=form,
                                   available_dates=available_iso, booked_dates=booked_iso)
        ...
        if booking.special_requests:
            note_parts.append(booking.special_requests)
        sel_date = (form.selected_date.data or '').strip()
        note_parts.append(f"Added Musician: {artist.get('name', 'Artist')} (+{add_price} KZT){' on ' + sel_date if sel_date else ''}")
        booking.special_requests = '\n'.join([p for p in note_parts if p])
        db.session.commit()
...
def create_tables():
    with app.app_context():
        ...
        # Ensure legacy databases have the new password column on user table
        try:
            inspector = inspect(db.engine)
            user_columns = [col["name"] for col in inspector.get_columns("user")]
            if "password" not in user_columns:
                # Add the password column for existing databases
                with db.engine.connect() as conn:
                    conn.execute(text("ALTER TABLE user ADD COLUMN password VARCHAR(200)"))
                    conn.commit()
        except Exception as e:
            # Log but don't crash the app; app will still work for fresh DBs
            print(f"Schema check/update for user.password failed: {e}")
...
def add_sample_data():
    ...
    venues_data = [
        {
            'name': 'Grand Hotel Mildom',
            'district': 'Medeu',
            'address': '172 Dostyk Avenue, Almaty',
            'address_link': 'https://go.2gis.com/L6SUk',
            ...
            'image_url': 'images/Grand Hotel mildm.png',
            'event_types': 'wedding,corporate,tusau_keser'
        },
        {
            'name': 'Abay',
            'district': 'Medeu',
            'address': 'Kok-Tobe 41/2 Omarova Street, Almaty',
            'address_link': 'https://go.2gis.com/lMkw5',
            ...
            'image_url': 'images/Abay restaurant.png',
            'event_types': 'wedding,tusau_keser,kudalyk'
        },
        {
            'name': 'Bakshasarai',
            'district': 'Bostandyk',
            'address': 'Timiryazev Street, 42 k1, Almaty',
            'address_link': 'https://go.2gis.com/Ggse8',
            ...
            'image_url': 'images/Bakshasarai.jpeg',
            'event_types': 'corporate,wedding'
        },
        {
            'name': 'Navat Restaurant',
            'district': 'Medeu',
            'address': '48 Dostyk Avenue, Almaty',
            'address_link': 'https://go.2gis.com/CIBgw',
        ...
        },
        {
            ...
            'district': 'Almaly',
            'address': 'Nurmakova Street, 25, Almaty',
            'address_link': 'https://go.2gis.com/0Ib6a',
        ...
        },
        {
            ...
            'district': 'Almaly',
            'address': '506 Seifullin Avenue, Almaty',
            'address_link': 'https://go.2gis.com/TnIMb',
    ...
    for venue_data in venues_data:
        ...
        # Add sample halls
        if venue.name == 'Grand Hotel Mildom':
            halls = [
...
def invitation_preview(token):
    ...
    # Generate invitation link with better cross-platform support
    invitation_link = url_for('guest_rsvp_page', token=token, _external=True)
    
    # Ensure the link uses the correct protocol and port
    if 'localhost' in invitation_link:
        # Replace localhost with the actual server IP for better mobile access
        import socket
        try:
            hostname = socket.gethostname()
            local_ip = socket.gethostbyname(hostname)
            invitation_link = invitation_link.replace('localhost', local_ip)
        except:
            pass  # Keep localhost if we can't get the IP
...
def guest_rsvp_page(token):
    ...
    print(f"RSVP route called with token: {token}")
    ...
    if request.method == 'POST':
        ...
        message_to_host = request.form.get('message_to_host', '').strip()
        
        print(f"Parsed data: name={name}, phone={phone}, rsvp_status={rsvp_status}, plus_one={plus_one}")
        
        # Simple validation
        if not name or not rsvp_status:
            flash('Please fill in your name and select your attendance status.', 'error')
        ...
        try:
            ...
            print(f"Guest created successfully! ID: {guest.id}")
...
if __name__ == '__main__':
    ...
    print("🚀 Event Planner is starting...")
    print("=" * 50)
    print("🌐 Local URL: http://localhost:5000")
    print("🔗 Network URL: http://192.168.0.16:5000")
    print("=" * 50)
    print("📱 To make it PUBLIC (so your friend can access it):")
    print("   1. Open a NEW terminal window")
    print("   2. Run: ngrok http 5000")
    print("   3. Share the ngrok URL with your friend!")
    print("=" * 50)
